...
# class nnUNetTrainer_Rand_CLDC1(nnUNetTrainer):
...
#                                        {},
#                                        {'iter_': 20, 'smooth': 1e-3, 'skeletonize_flag': 'rand', 'original_algo':True},
#                                     weight_ce=lambda_ce, 
...
class nnUNetTrainer_Rand_CLDC2(nnUNetTrainer):
    def _build_loss(self):
        lambda_cldice = 1.0
        lambda_dice = 1.0
        lambda_ce = lambda_dice + lambda_cldice
        ...
        loss = DC_and_CE_and_CLDC_loss({'batch_dice': self.configuration_manager.batch_dice, 'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, 
                                       {},
                                       {'iter_': 20, 'smooth': 1e-3, 'skeletonize_flag': 'rand', 'original_algo':False},
                                    weight_ce=lambda_ce, 
                                    weight_dice=lambda_dice, 
                                    weight_cldice=lambda_cldice, 
                                    ignore_label=self.label_manager.ignore_label, 
