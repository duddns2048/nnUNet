...
def maybe_load_checkpoint(nnunet_trainer: nnUNetTrainer, continue_training: bool, validation_only: bool,
    ...
    if continue_training:
        ...
        if validation_ckpt != None :
            expected_checkpoint_file = join(nnunet_trainer.output_folder, f'checkpoint_latest_{validation_ckpt}.pth')
        else:
            expected_checkpoint_file = join(nnunet_trainer.output_folder, 'checkpoint_final.pth')
        if not isfile(expected_checkpoint_file):
...
def run_training(dataset_name_or_id: Union[str, int],
    ...
    if num_gpus > 1:
        ...
        maybe_load_checkpoint(nnunet_trainer, continue_training, only_run_validation, pretrained_weights, validation_ckpt)
        ...
        if val_with_best:
            nnunet_trainer.load_checkpoint(join(nnunet_trainer.output_folder, 'checkpoint_best.pth'))
        nnunet_trainer.perform_actual_validation(export_validation_probabilities, validation_ckpt)
...
def run_training_entry():
    ...
    parser.add_argument('-exp_name', default=None, type=str,
                        help='exp name')
    parser.add_argument('-validation_ckpt', default=-1, type=int,
                        help='validation ckpt number')
